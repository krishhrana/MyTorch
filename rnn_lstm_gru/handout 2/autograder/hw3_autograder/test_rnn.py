...
class RNNTest(Test):
    ...
    def test_rnn_classifier(self):
        ...
        print("observed", my_dh[0][0], "\n", "expected", dh.detach().numpy()[0][0])
        ...
        for l, rnn_cell in enumerate(my_rnn_model.rnn):
            print("MY Model: ", my_rnn_model.rnn[l].dW_hh)
            print("Expected: ", grad_dict["rnn.weight_hh_l%d" % l].detach().numpy())
            assert np.allclose(
    ...
    def gen_test(self):
        file = os.path.join(
                    "hw3_autograder", "data", "toy_rnncell_backward.npy"
                )
