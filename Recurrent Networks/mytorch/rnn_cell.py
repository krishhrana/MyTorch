...
class RNNCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        h_t = self.activation(x @ self.W_ih.transpose((1, 0)) + self.b_ih + h_prev_t @ self.W_hh.transpose((1, 0)) + self.b_hh) # TODO
    ...
    def backward(self, delta, h_t, h_prev_l, h_prev_t):
        ...
        self.dW_ih += (dz.T @ h_prev_l) / delta.shape[0] # TODO
        self.dW_hh += (dz.T @ h_prev_t) / delta.shape[0] # TODO
        self.db_ih += np.sum(dz, axis = (0)) / delta.shape[0] # TODO
        self.db_hh += np.sum(dz, axis = (0)) / delta.shape[0] # TODO
