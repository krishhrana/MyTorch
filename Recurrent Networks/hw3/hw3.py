...
class CharacterPredictor(object):
    ...
    def forward(self, x, h):
        ...
        proj_input = hnext.reshape(1, -1)# TODO
        ...
        logits = self.projection.forward(proj_input) # TODO
...
def inference(net, inputs):
    ...
    h = np.zeros(shape=(net.hidden_dim))
    logits = np.zeros(shape=(seq_length, num_classes))

    for seq in range(seq_length):
        logits[seq], h = net.forward(inputs[seq], h)
